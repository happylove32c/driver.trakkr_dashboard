...
export default function PickupPage() {
  ...
  const handleCapture = async (file: File) => {
    ...
    const path = `${jobId}/pickup_${Date.now()}.jpg`
  ...
  const handleConfirm = async () => {
    ...
    if (!error) {
      router.push(`/jobs/${jobId}/navigation`)
    } else {
  ...
  if (!job) {
    return (
      <div className="view active">
        <TopBar title="Confirm Pickup" backHref={`/jobs/${jobId}`} />
    ...
    <div className="view active">
      ...
      <div className="content-area">
        ...
        <div style={{ margin: '20px 0' }}>
          <PhotoCapture label="Pickup Photo" onCapture={handleCapture} captured={photoCaptured} />
